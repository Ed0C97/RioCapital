...
def get_articles():
    try:
        ...
        page = request.args.get('page', 1, type=int)
        per_page = request.args.get('per_page', 12, type=int)  # Aumentato a 12 per l'archivio
        category_slug = request.args.get('category_slug', type=str)
        author_id = request.args.get('author_id', type=int)
        search = request.args.get('search', '')
        published_only = request.args.get('published', 'true').lower() == 'true'
        ...
        # --- 2. AGGIUNGI LA LETTURA DEI NUOVI FILTRI DI DATA ---
        ...
        # --- 3. AGGIUNGI LA LOGICA DI FILTRO PER DATA ---
        if year:
