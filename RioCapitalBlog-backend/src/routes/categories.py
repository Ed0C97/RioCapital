...
@categories_bp.route('/<int:category_id>', methods=['GET']) # URL: GET /api/categories/1
...
def create_category():
    try:
        ...
        db.session.commit()
        return jsonify({'message': 'Categoria creata con successo', 'category': category.to_dict()}), 201
    except Exception as e:
        db.session.rollback()
        return jsonify({'error': str(e)}), 500

@categories_bp.route('/<int:category_id>', methods=['PUT']) # URL: PUT /api/categories/1
@author_required
def update_category(category_id):
    try:
        ...
        db.session.commit()
        return jsonify({'message': 'Categoria aggiornata con successo', 'category': category.to_dict()}), 200
    except Exception as e:
        db.session.rollback()
        return jsonify({'error': str(e)}), 500

@categories_bp.route('/<int:category_id>', methods=['DELETE']) # URL: DELETE /api/categories/1
@author_required
def delete_category(category_id):
    try:
        ...
        if articles_count > 0:
            return jsonify({'error': f'Impossibile eliminare la categoria: ci sono {articles_count} articoli associati'}), 400
...
@categories_bp.route('/<string:slug>', methods=['GET']) # URL: GET /api/categories/nome-slug
