...
def author_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not user or not user.can_write_articles():
            return (
                jsonify({"error": "Accesso negato: privilegi di scrittura richiesti"}),
                403,
            )
        return f(*args, **kwargs)
...
def register():
    try:
        ...
        if (
            ...
            return (
                jsonify({"error": "Username, email e password sono obbligatori"}),
                400,
            )
...
def login():
    try:
        ...
        if user and user.check_password(data["password"]) and user.is_active:
            ...
            return (
                jsonify(
                    {"message": "Login effettuato con successo", "user": user.to_dict()}
                ),
                200,
            )
        else:
...
def complete_oauth_registration():
    ...
    try:
        ...
        print(
            f"Step 1 OK: Trovato oauth_profile in sessione: {session['oauth_profile']}"
        )
        ...
        print(
            f"Step 5 OK: Sessione pulita e login effettuato per user_id: {new_user.id}"
        )
