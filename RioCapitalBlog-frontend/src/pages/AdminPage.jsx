...
const OverviewStatCard = ({ title, value, icon: Icon, color, bgColor, growth }) => (
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          {growth !== undefined && growth !== 0 && (
            <div className={`flex items-center space-x-1 text-sm ${parseFloat(growth) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
    ...
    <div className="group border-b last:border-b-0 transition-colors py-8">
        <div className="flex items-center justify-between">
            ...
            <div className="flex-1 min-w-0">
                ...
                <h3 className="text-lg font-semibold text-gray-800 mb-3 line-clamp-1">{article.title}</h3>
                <div className="flex items-center space-x-5 text-sm text-muted-foreground">
                    ...
                    <div className="flex items-center space-x-1"><Heart className="w-4 h-4" /><span>{formatNumber(article.likes_count)}</span></div>
                    <div className="flex items-center space-x-1"><MessageSquare className="w-4 h-4" /><span>{formatNumber(article.comments_count)}</span></div>
                </div>
            </div>
...
const ArticleList = ({ articles, categories, onTogglePublish, onDelete, page, totalPages, goToPage, pageInput, setPageInput }) => {  const [searchTerm, setSearchTerm] = useState('');
  ...
  const filteredArticles = useMemo(() => {
    return articles
      .filter(article => {
        ...
        const statusMatch = statusFilter === 'all' || (statusFilter === 'published' && article.published) || (statusFilter === 'draft' && !article.published);
  ...
  const groupedArticles = useMemo(() => {
    return filteredArticles.reduce((acc, article) => {
      try {
        ...
        console.warn(`Data non valida per l'articolo ID ${article.id}:`, article.created_at);
        if (!acc['Senza data']) { acc['Senza data'] = []; }
      ...
      <CardContent className="px-0">
        <div className="flex flex-col md:flex-row gap-6 mb-8 p-0 trasparent">
          <div className="relative flex-1">
            ...
            <Input placeholder="Cerca per titolo..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10" />
          </div>
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            <SelectTrigger className="w-full md:w-48"><SelectValue placeholder="Stato" /></SelectTrigger>
            <SelectContent>
        ...
        <div className="space-y-12">
          {filteredArticles.length > 0 ? (
            Object.entries(groupedArticles).map(([monthYear, articlesInGroup]) => (
              <div key={monthYear}>
                <h2 className="text-2xl font-semibold mb-6 capitalize text-gray-800">{monthYear}</h2>
            ...
            <div className="text-center py-16 bg-white border rounded-lg">
              ...
              <p className="text-muted-foreground mt-2">Prova a modificare i filtri o a creare un nuovo articolo.</p>
            </div>
          )}
        </div>
      {totalPages > 1 && (
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const articlesRes = await fetch(`/api/articles/my-articles?page=${page}`, { credentials: 'include' });
        ...
        if (articlesRes.ok) {
          ...
          console.error("Errore API articoli:", articlesRes.status, await articlesRes.text());
          toast.error(`Errore nel caricamento degli articoli (status: ${articlesRes.status}).`);
        }
...
// Aggiungi 'page' qui, così il fetch viene rieseguito quando cambi pagina
  ...
  const handleTogglePublish = async (articleId, currentStatus) => {
    try {
      ...
      if (response.ok) {
        ...
        toast.success(`Articolo ${currentStatus ? 'impostato come bozza' : 'pubblicato'}.`);
      } else {
  ...
  const handleDeleteArticle = async (articleId, articleTitle) => {
    if (!confirm(`Sei sicuro di voler eliminare l'articolo "${articleTitle}"? L'azione è irreversibile.`)) return;
    try {
  ...
  const overviewStats = useMemo(() => [
    { title: 'Articoli', value: formatNumber(analytics.overview?.totalArticles), icon: FileText, color: 'text-blue-600', bgColor: 'bg-blue-100', growth: getGrowthPercentage(analytics.overview?.totalArticles, analytics.overview?.previousArticles) },
    { title: 'Utenti', value: formatNumber(analytics.overview?.totalUsers), icon: Users, color: 'text-green-600', bgColor: 'bg-green-100', growth: getGrowthPercentage(analytics.overview?.totalUsers, analytics.overview?.previousUsers) },
    { title: 'Commenti', value: formatNumber(analytics.overview?.totalComments), icon: MessageSquare, color: 'text-purple-600', bgColor: 'bg-purple-100', growth: getGrowthPercentage(analytics.overview?.totalComments, analytics.overview?.previousComments) },
    { title: 'Donazioni', value: formatCurrency(analytics.overview?.totalRevenue), icon: DollarSign, color: 'text-yellow-600', bgColor: 'bg-yellow-100', growth: getGrowthPercentage(analytics.overview?.totalRevenue, analytics.overview?.previousRevenue) }
  ], [analytics.overview]);
        ...
        <div className="w-full mb-12">
          <div className="max-w-[1012px] mx-auto px-[16px] sm:px-[16px] lg:px-[16px] pt-12">
            ...
            <div className="mb-8">
              ...
              <h2 className="text-2xl font-regular text-gray-500">Admin Dashboard</h2>
            </div>

            {/* Controlli Header */}
            <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold mb-2">Welcome, {(user?.first_name || user?.last_name) ? `${user?.first_name || ''} ${user?.last_name || ''}`.trim() : user?.username}</h1>
              {/* Questo testo ora cambia in base alla tab attiva */}
              <p className="text-muted-foreground">
                {tabs.find(tab => tab.value === activeTab)?.label || 'Panoramica e gestione del blog.'}
              </p>
            </div>
              <div className="flex items-center space-x-4">
                <Button onClick={() => navigate('/admin/articoli/nuovo')} className="btn-outline btn-outline-blue">
            ...
            {activeTab === 'overview' && (
              <div className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  {overviewStats.map((stat, i) => <OverviewStatCard key={i} {...stat} />)}
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <Card className="shadow-none border-none">
                    <CardHeader>
                      <CardTitle className="text-xl">Articoli nel Tempo</CardTitle>
                    </CardHeader>
                    <CardContent>
            ...
            {activeTab === 'analytics' && (
              <div className="space-y-8">
                <Card className="shadow-none border-none">
                  <CardHeader>
                    <div className="flex flex-wrap items-center justify-between gap-4">
                      <CardTitle className="text-2xl">Dettagli Analytics</CardTitle>
                      <div className="flex items-center space-x-2">
                        <Select value={timeRange} onValueChange={(v) => { setTimeRange(v); fetchAnalytics(v); }}>
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button variant="outline" className="btn-outline btn-outline-gray">
                              <Download className="w-4 h-4 mr-2" />Export
                            </Button>
                          </DropdownMenuTrigger>
                          <DropdownMenuContent>
                            <DropdownMenuItem onClick={() => handleExport('overview')}>Panoramica</DropdownMenuItem>
                            <DropdownMenuItem onClick={() => handleExport('articles')}>Articoli</DropdownMenuItem>
                            <DropdownMenuItem onClick={() => handleExport('users')}>Utenti</DropdownMenuItem>
