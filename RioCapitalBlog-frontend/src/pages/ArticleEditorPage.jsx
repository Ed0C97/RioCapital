...
import {
  Save, Eye, ArrowLeft, CheckCircle, Link as LinkIcon, Type, Tag, ArrowRight
} from 'lucide-react';
...
const ArticleEditorPage = () => {
  ...
  const fetchArticle = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/articles/id/${id}`, { credentials: 'include' });
      if (!response.ok) {
  ...
  const generateSlug = (title) => {
    return title.toLowerCase().replace(/[àáâãäå]/g, 'a').replace(/[èéêë]/g, 'e').replace(/[ìíîï]/g, 'i').replace(/[òóôõö]/g, 'o').replace(/[ùúûü]/g, 'u').replace(/[ç]/g, 'c').replace(/[ñ]/g, 'n').replace(/[^a-z0-9\s-]/g, '').replace(/\s+/g, '-').replace(/-+/g, '-').replace(/^-+|-+$/g, '');
  };
  ...
  const validateArticle = () => {
    ...
    if (article.title.length > 200) errors.push('Title is too long (max 200 chars).');
    if (article.excerpt.length > 500) errors.push('Excerpt is too long (max 500 chars).');
    return errors;
  };
  ...
  const handleSave = async (publish = false) => {
    ...
    setSaving(true);
    try {
      const articleData = { ...article, published: publish, tags: article.tags.split(',').map(tag => tag.trim()).filter(tag => tag) };
  ...
  const cleanInputStyle = "!border !border-gray-300 !bg-white focus:border-blue-500 focus:ring-blue-500";
      ...
      <div className="bg-[#f5f5f7]">
        ...
        <div className="max-w-[1012px] mx-auto px-[16px] sm:px-[16px] lg:px-[16px] pb-16 space-y-8">
          ...
          <div className="text-sm text-muted-foreground space-y-4">
            ...
            <div className="flex items-center gap-10">
              <div className="flex items-center gap-2">
                <span>Status:</span>
                <span className="font-semibold text-[#0071e3]">{article.published ? 'Published' : 'Draft'}</span>
              </div>
              <div className="flex items-center gap-2">
                <span>Featured:</span>
                <span className="font-semibold text-[#E18528FF]">{article.featured ? 'Yes' : 'No'}</span>
              </div>
              <div className="flex items-center gap-2">
                <span>Contacts:</span>
                <span className="font-semibold text-[#499537FF]">{article.show_author_contacts ? 'Visible' : 'Hidden'}</span>
              </div>
            </div>
          </div>
          ...
          <Card className="shadow-none border-none">
            <CardHeader><CardTitle className="text-xl">Main Settings</CardTitle></CardHeader>
            <CardContent className="p-6 space-y-6">
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  {validationAttempted && !article.title.trim() && <p className="text-xs text-red-500 italic mt-2">Title is required.</p>}
                </div>
                <div className="space-y-2">
                  ...
                  />
                  {validationAttempted && !article.excerpt.trim() && <p className="text-xs text-red-500 italic mt-2">Excerpt is required.</p>}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2 w-full">
                  ...
                  {validationAttempted && !article.category_id && <p className="text-xs text-red-500 italic mt-2">A category selection is required.</p>}
                </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between"><Label htmlFor="published">Published</Label><div className="switch-container"><Switch id="published" checked={article.published} onCheckedChange={(checked) => handleInputChange('published', checked)} /></div></div>
                {isAdmin() && (<div className="flex items-center justify-between"><Label htmlFor="featured">Featured</Label><div className="switch-container"><Switch id="featured" checked={article.featured} onCheckedChange={(checked) => handleInputChange('featured', checked)} /></div></div>)}
                <div className="flex items-center justify-between"><Label htmlFor="show-contacts">Show Contacts</Label><div className="switch-container"><Switch id="show-contacts" checked={article.show_author_contacts} onCheckedChange={(checked) => handleInputChange('show_author_contacts', checked)} /></div></div>
              </div>
            </CardContent>
          </Card>
          ...
          {/* Cover Image Card */}
          <Card className="shadow-none border-none">
            <CardHeader><CardTitle className="text-xl">Cover Image</CardTitle></CardHeader>
            <CardContent>
          ...
          <Card className="shadow-none border-none">
            <CardHeader><CardTitle className="text-xl">Article Content <span className="text-red-500">*</span></CardTitle></CardHeader>
            <CardContent className="p-6 pt-0">
              ...
              />
              {validationAttempted && !article.content.trim() && <p className="text-xs text-red-500 italic mt-2">Content is required.</p>}
            </CardContent>
          </Card>
