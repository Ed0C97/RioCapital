...
import { Heart, Coffee, Hamburger, Star, BadgeInfo, Beer, UtensilsCrossed, Check, Coins, WalletCards, Feather } from 'lucide-react';
...
const DonationFormWrapper = ({ amount, donorInfo, termsAccepted, paymentMethod, children }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/stripe/create-checkout-session', {
        ...
        body: JSON.stringify({ amount: parseFloat(amount ), paymentMethod: paymentMethod }),
      });
...
const DonatePage = () => {
  ...
  const [cardDetails, setCardDetails] = useState({ number: '', name: '', exp: '', ccv: '' });
  ...
  const getCardType = (cardNumber) => {
    ...
    if (mastercardPrefixes.some(prefix => cleaned.startsWith(prefix))) return 'mastercard';
    const mcRangePrefix = parseInt(cleaned.substring(0, 4));
    if (mcRangePrefix >= 2221 && mcRangePrefix <= 2720) return 'mastercard';
    return 'default';
  };
  ...
  const handleCardInputChange = (e) => {
    ...
    switch (key) {
      ...
      case 'exp':
        ...
        processedValue = cleaned.length >= 3 ? `${cleaned.slice(0, 2)}/${cleaned.slice(2, 4)}` : cleaned;
        break;
      case 'ccv':
    ...
    setCardDetails(prevDetails => ({ ...prevDetails, [key]: processedValue }));
  };

  const getFinalAmount = () => customAmount || selectedAmount;

  return (
    <div style={{ background: 'linear-gradient(-70deg, #202020, #000000)' }}>
      <FadeInOnScroll>
        <div className="w-full mb-12">
          <div className="max-w-[1012px] mx-auto px-[16px] sm:px-[16px] lg:px-[16px] pt-12">
            <div className="mb-8">
              <div className="border-b border-[#d2d2d7] my-2"></div>
              <h2 className="text-2xl font-regular text-white">Support with a ...</h2>
            </div>
                    ...
                    <div className="space-y-6 mt-36">
                      {donorInfo.message && (
                        <div className="text-white mb-8">
                          <strong className="font-bold">Your message</strong>
                          <p className="whitespace-pre-wrap break-words text-justify text-white/80 mt-2">
                            {donorInfo.message}
                          </p>
                        </div>
                      )}
