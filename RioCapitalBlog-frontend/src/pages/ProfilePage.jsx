...
import { User, Mail, Linkedin, Bell, Save, Shield, Calendar } from 'lucide-react';
      ...
      <div className="w-full mb-12">
        <div className="max-w-[1012px] mx-auto px-[16px] sm:px-[16px] lg:px-[16px] pt-12">
          ...
          <h2 className="text-2xl font-regular">
            <span className="text-gray-500">Welcome</span>, <span className="text-gray-800">{user.username}</span>
          </h2>
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-4">
                ...
                />
                <div>
                  <h3 className="text-xl font-bold">{formData.first_name} {formData.last_name}</h3>
              ...
              <div className="hidden md:flex flex-col items-end text-sm text-muted-foreground space-y-1">
                {/* Badge Ruolo */}
                <div className={`inline-flex items-center px-3 py-1 rounded-full font-medium border ${getRoleBadgeColor(user.role)}`}>
            ...
            <div className="space-y-6">
              <h3 className="text-xl font-semibold border-b pb-2">Informazioni Personali</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <Label htmlFor="first_name">Nome</Label>
                  <div className="relative">
                    <User className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-4 h-4" />
                    <Input id="first_name" name="first_name" value={formData.first_name} onChange={handleInputChange} className="pl-9 !bg-white" />
                  </div>
                </div>
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <User className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-4 h-4" />
                    <Input id="last_name" name="last_name" value={formData.last_name} onChange={handleInputChange} className="pl-9 !bg-white" />
                  </div>
                </div>
              </div>
              <div className="space-y-2">
                <Label htmlFor="bio">Biografia</Label>
                <Textarea id="bio" name="bio" value={formData.bio} onChange={handleInputChange} rows={4} className="!bg-white shadow-none border-none" />
              </div>
              <div className="space-y-2">
            ...
            {/* Sezione Preferenze */}
            <div className="space-y-6">
              <h3 className="text-xl font-semibold border-b pb-2">Preferenze</h3>
              <div className="flex items-center justify-between">
                <div>
                  <Label htmlFor="newsletter">Iscrizione Newsletter</Label>
                  <p className="text-sm text-muted-foreground">Ricevi aggiornamenti e analisi via email.</p>
                </div>
                <div className="switch-container">
                  <Switch id="newsletter" checked={formData.newsletter_subscribed} onCheckedChange={handleSwitchChange} />
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-end">
              <a
                ...
                className={`flex items-center text-sm font-medium transition-colors ${
                  loading ? 'text-gray-400 cursor-not-allowed' : 'text-gray-500 hover:text-blue-600'
                }`}
              >
