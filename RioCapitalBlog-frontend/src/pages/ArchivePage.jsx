...
const ArchiveListItem = ({ article, isLast }) => {
  ...
  return (
    ...
    <Link to={`/article/${article.slug}`} className={clsx("archive-item-link", { 'no-divider': isLast })}>
...
const ArchivePage = () => {
      ...
      try {
        if (!filterOptions.categories.length) {
          ...
          if (filtersResponse.ok) {
            ...
            console.error('Errore nel caricamento dei filtri:', filtersResponse.statusText);
          }
        }
  ...
  const getActiveFilterLabels = () => {
    ...
    if (categorySlug) {
      const cat = filterOptions.categories.find(c => c.value === categorySlug);
  ...
  if (loading && !articles.length) return <div className="text-center p-20">Caricamento...</div>;
    ...
    <div className="bg-white w-full">
      <div className="mx-auto px-4 py-12" style={{ maxWidth: '1012px' }}>
          ...
          <FadeInOnScroll delay={100}>
            <div className="mb-8">
              <div className="flex items-center gap-2 mb-4 flex-wrap">
                <Button onClick={toggleFilters} variant="outline" className="flex items-center gap-2 self-start">
                  {isFilterPanelOpen ? <X className="w-4 h-4" /> : <Filter className="w-4 h-4" />}
                  <span>Filtri</span>
              ...
              <div className={twMerge(
                ...
                <div className={twMerge(
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-4">
                    {/* Colonne Filtri */}
                    <div>
                      <h3 className="text-xs font-semibold text-gray-500 mb-3 uppercase">Categorie</h3>
                      <ul className="space-y-2">
                        <li><FilterLink label="Tutte le categorie" onClick={() => handleFilterChange('category', 'all')} isActive={!getActiveFilter('category')} /></li>
                        {filterOptions.categories?.map(cat => (
                          <li key={cat.value}><FilterLink label={cat.label} onClick={() => handleFilterChange('category', cat.value)} isActive={getActiveFilter('category') === cat.value} /></li>
                        ))}
                      </ul>
                    </div>
                    <div>
                      <h3 className="text-xs font-semibold text-gray-500 mb-3 uppercase">Anno</h3>
                      <ul className="space-y-2">
                        <li><FilterLink label="Tutti gli anni" onClick={() => handleFilterChange('year', 'all')} isActive={!getActiveFilter('year')} /></li>
                        {Object.keys(filterOptions.dates || {}).sort((a, b) => b - a).map(year => (
                          <li key={year}><FilterLink label={year} onClick={() => handleFilterChange('year', year)} isActive={getActiveFilter('year') === year} /></li>
                        ))}
                      </ul>
                    </div>
                    <div>
                      <h3 className="text-xs font-semibold text-gray-500 mb-3 uppercase">Mese</h3>
                      {getActiveFilter('year') ? (
                        <ul className="space-y-2">
                          <li><FilterLink label="Tutti i mesi" onClick={() => handleFilterChange('month', 'all')} isActive={!getActiveFilter('month')} /></li>
                          {filterOptions.dates[getActiveFilter('year')]?.map(month => (
                            <li key={month}><FilterLink label={months[month - 1]} onClick={() => handleFilterChange('month', String(month))} isActive={getActiveFilter('month') === String(month)} /></li>
                          ))}
                        </ul>
                      ) : (
                        <p className="text-sm text-gray-400 italic mt-2 py-1.5">Seleziona prima un anno</p>
                      )}
                    </div>
                  </div>
        ...
        <div className="space-y-12">
          {groupKeys.map((monthYear, groupIndex) => (
              ...
              <div>
                ...
                <div className="archive-list">
                  {groupedArticles[monthYear].map((article, articleIndex) => {
                    ...
                    const isLastInGroup = articleIndex === groupedArticles[monthYear].length - 1;
                    const isLastGroupOnPage = groupIndex === groupKeys.length - 1;
                    const isLast = isLastInGroup && isLastGroupOnPage;

                    return (
                      <FadeInOnScroll key={article.id} delay={articleIndex * 80}>
                        ...
                        <ArchiveListItem article={article} isLast={isLastInGroup} />
                      </FadeInOnScroll>
                    );
                  })}
                </div>
        ...
        <FadeInOnScroll>
          <div className="pagination-container">
            <button className="pagination-button" onClick={() => goToPage(page - 1)} disabled={page <= 1} aria-label="Pagina precedente">
            ...
            <button className="pagination-button" onClick={() => goToPage(page + 1)} disabled={page >= totalPages} aria-label="Pagina successiva">
