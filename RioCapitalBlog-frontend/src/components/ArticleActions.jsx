...
const ArticleActions = ({ article, onUpdate, size = 'default' }) => {
  ...
  const handleLike = async (e) => {
    ...
    try {
      ...
      if (onUpdate) {
        onUpdate({ ...article, likes_count: data.likes_count, user_has_liked: data.liked });
      }
    } catch (error) {
  ...
  return (
    <div className={clsx("flex items-center space-x-4", isSmall ? "text-xs" : "text-sm")}>
      {/* Pulsante Mi Piace */}
      <div
        onClick={handleLike}
        role="button"
        ...
        <Heart
          className={clsx(
            ...
            "transition-all duration-200",
            {
              ...
              "text-[#a8192f] fill-transparent stroke-current group-hover:fill-[#a8192f] group-hover:stroke-[#a8192f]": !hasLiked,
            }
          )}
          strokeWidth={1.5}
        />
        <span className={clsx("font-medium", { "text-[#a8192f]": hasLiked, "group-hover:text-[#a8192f]": !hasLiked })}>{likes}</span>
      </div>
      ...
      <div className="flex items-center space-x-1.5 group text-gray-500 hover:text-[#0e549c] transition-colors duration-200">
        <MessageCircle
          className={clsx(iconSize, "text-[#0e549c] fill-transparent stroke-current group-hover:fill-[#0e549c] group-hover:stroke-[#0e549c] transition-all duration-200")}
          strokeWidth={1.5}
        />
