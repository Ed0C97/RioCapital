...
import { Search, Menu, X, User, LogOut, Heart, PenTool, BadgeEuro, LayoutDashboard, ArrowRight } from 'lucide-react';
...
const Header = () => {
  ...
  const handleFormSubmit = (e) => {
    ...
    if (query) {
      ...
      else if (results.length > 0 && results[0].score < 0.1) navigate(results[0].item.slug);
  ...
  const showSearchResults = isSearchVisible && searchQuery.trim().length > 0 && results.length > 0;
  const quickLinks = [
    { name: 'Home', path: '/' },
          ...
          <div className="max-w-[1012px] mx-auto px-[16px] h-full flex flex-col">
            <div className="h-14 flex items-center justify-between flex-shrink-0 relative">
              ...
              <div className={twMerge(
                ...
                <div onClick={toggleSearch} className="cursor-pointer text-gray-500 hover:text-gray-900 p-2">
              ...
              <div className={twMerge(
                ...
                <nav className="flex items-center gap-8">
                  <Link to="/profile" onClick={closeAllMenus} className="flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-gray-900"><User className="w-4 h-4" />Profile</Link>
                  <Link to="/favorites" onClick={closeAllMenus} className="flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-gray-900"><Heart className="w-4 h-4" />Favorites</Link>
                  {canWriteArticles() && (
                    <Link to="/admin/articoli/nuovo" onClick={closeAllMenus} className="flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-gray-900"><PenTool className="w-4 h-4" />New Article</Link>
                  )}
                  {isAdmin() && (
                    <Link to="/admin/dashboard" onClick={closeAllMenus} className="flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-gray-900"><LayoutDashboard className="w-4 h-4" />Dashboard</Link>
                  )}
                </nav>
                <div className="flex items-center gap-8 ml-8">
                  <a href="#" onClick={(e) => { e.preventDefault(); handleLogout(); }} className="flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-gray-900"><LogOut className="w-4 h-4" />Logout</a>
                  <a href="#" onClick={(e) => { e.preventDefault(); toggleProfileNav(); }} className="text-gray-500 hover:text-gray-900"><X className="w-5 h-5" /></a>
                </div>
              </div>
            </div>
            ...
            <div className={twMerge(
              ...
              <form onSubmit={handleFormSubmit} className="relative flex items-center gap-4 mb-12">
              ...
              <div className={clsx("transition-all duration-300 ease-out", isContentVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4")}>
                {showSearchResults ? (
                  <SearchResultsDropdown results={results} onResultClick={handleResultClick} isExpandedView={true} />
                ) : (
                  <div>
                    <h3 className="text-xs font-regular text-gray-500 mb-3">Quick links</h3>
                    <ul className="space-y-3">
                      {quickLinks.map((link, index) => (
                        <li key={link.name} style={{ transitionDelay: `${index * 50}ms` }} className={clsx("transition-all duration-300 ease-out", isContentVisible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4")}>
                          <Link to={link.path} onClick={closeSearch} className="flex items-center gap-3 text-sm text-gray-700 hover:text-blue-600 group">
